...
public class CerrarPrueba extends JFrame {
	...
	public CerrarPrueba(int idprueba) {
		...
		lblidoro.setBounds(27, 251, 109, 14);
		...
		lblidplata.setBounds(27, 251, 109, 14);
		...
		lblidbronce.setBounds(27, 251, 109, 14);
...
/**conseguir id metales*/		
		
		
		DefaultComboBoxModel<Metal> idmetal = new DefaultComboBoxModel<Metal>();
		MetalDAO mDAO = new MetalDAO(ConexBD.getCon());
		ArrayList<Metal> idlist = (ArrayList<Metal>) mDAO.buscarTodos();

		String[] metal = new String[metalList.size()];
		for (int i = 0; i < metalList.size(); i++)
			metal[i] = metalList.get(i).getId().data();


		JComboBox<Metal> idoro = new JComboBox<Metal>();
		idoro.setModel(new DefaultComboBoxModel(metal));		
		lblidoro.setLabelFor(idoro);
		idoro.setBounds(140, 247, 287, 22);
		contentPane.add(idoro);

		JComboBox<Metal> idplata = new JComboBox<Metal>();
		idplata.setModel(new DefaultComboBoxModel(metal));
		lblidplata.setLabelFor(idplata);
		idplata.setBounds(140, 278, 287, 22);
		contentPane.add(idplata);

		JComboBox<Metal> idbronce = new JComboBox<Atleta>();
		idbronce.setModel(new DefaultComboBoxModel(metal));
		lblidbronce.setLabelFor(idbronce);
		idbronce.setBounds(140, 309, 287, 22);
		contentPane.add(idbronce);
...
/** comprobacion de los datos seleccionados en los combobox de los puestos, chequeando el id del atleta **/				
				valido = false;
				
				/** comprobando el puesto 1*/
				valido = (comboBoxPuesto1.getSelectedIndex() != -1);
				if (!valido) {
					errores += "Debe seleccionar los datos correctos.\n";
					...
					Atleta atleta = aDAO.buscarPorID(idAtleta);
					ConexBD.cerrarConexion();
					if (atleta == null)
						valido = false;
					else
...
/** comprobacion de los id seleccionados en cada puesto **/				
				
				/**comprobacion id oro*/
				valido = false;
				valido = (idoro.getSelectedIndex() != -1);
				if (!valido) {
					errores += "Debe introducir el dato correcto para id.\n";
					lblidoro.setForeground(Color.RED);
				} else {
					MetalDAO mDAO = new MetalDAO(ConexBD.getCon());
					String seleccionado = (String) idoro.getSelectedItem();
					String[] aux = seleccionado.split("\\.");
					long idMetal = Long.valueOf(aux[0]);
					Metal metal = mDAO.buscarPorID(idMetal);
					ConexBD.cerrarConexion();
					if (metal == null)
						valido = false;
					else
						nueva.setMetal(metal);
				}
			
				
				
				/**comprobacion id plata*/
				valido = false;
				valido = (idplata.getSelectedIndex() != -1);
				if (!valido) {
					errores += "Debe introducir el dato correcto para id.\n";
					lblidplata.setForeground(Color.RED);
				} else {
					MetalDAO mDAO = new MetalDAO(ConexBD.getCon());
					String seleccionado = (String) idplata.getSelectedItem();
					String[] aux = seleccionado.split("\\.");
					long idMetal = Long.valueOf(aux[0]);
					Metal metal = mDAO.buscarPorID(idMetal);
					ConexBD.cerrarConexion();
				...
				/**comprobacion id bronce*/
				valido = false;
				valido = (idbronce.getSelectedIndex() != -1);
				if (!valido) {
					errores += "Debe introducir el dato correcto para id.\n";
					lblidbronce.setForeground(Color.RED);
				} else {
					MetalDAO mDAO = new MetalDAO(ConexBD.getCon());
					String seleccionado = (String) idbronce.getSelectedItem();
					String[] aux = seleccionado.split("\\.");
					long idMetal = Long.valueOf(aux[0]);
					Metal metal = mDAO.buscarPorID(idMetal);
					ConexBD.cerrarConexion();
		...
		public String data() {
			return "resultado de la prueba" + prueba.getId() + " ' " + prueba.getNombre() + " ' " + celebrado el pasado + "|"
					+ prueba.getFecha().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")) + "en" + prueba.getLugar();
		}
		
		/***exportacion */
		private static void exportar(Resultado[] resultado) {
			String path = "resultado_prueba<idPrueba>.txt";
